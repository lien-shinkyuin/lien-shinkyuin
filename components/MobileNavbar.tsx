...
const MobileNavbar = () => {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 z-50 w-full h-16 bg-white border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
      <div className="grid h-full grid-cols-4 mx-auto font-medium">
        {menuItems.map((item, index) => (
          ...
          >
            <div className="text-stone-500 group-hover:text-emerald-700">
              {item.icon}
            </div>
            <span className="text-[10px] mt-1 text-stone-500 group-hover:text-emerald-700">
              {item.label}
            </span>
          </a>
        ))}
